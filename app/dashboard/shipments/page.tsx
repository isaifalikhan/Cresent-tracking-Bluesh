    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">Track and manage your incoming and outgoing packages.</p>
        </div>
      ...
      <div className="grid gap-4">
        <div className="bg-card/50 border border-border rounded-xl p-4 flex gap-4">
          <div className="relative flex-1">
            <Search className="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground" />
            <input 
              type="text" 
              placeholder="Search by tracking ID, origin, or destination..." 
              className="w-full bg-background border border-border rounded-lg pl-9 pr-4 py-2 text-sm text-foreground placeholder-muted-foreground focus:outline-none focus:ring-1 focus:ring-blue-500"
            />
          </div>
          <button className="px-4 py-2 bg-background border border-border rounded-lg text-muted-foreground hover:text-foreground hover:border-foreground/20 transition-colors flex items-center gap-2 text-sm">
        ...
        <div className="space-y-4">
          {shipments.slice(0, 5).map((shipment, index) => (
            <motion.div
              ...
              className="bg-card/50 border border-border rounded-xl p-6 hover:border-foreground/20 transition-all group"
            >
              <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
                <div className="flex items-start gap-4">
                  <div className="w-12 h-12 rounded-xl bg-accent flex items-center justify-center text-blue-600 dark:text-blue-400 group-hover:text-white group-hover:bg-blue-600 transition-colors">
                  ...
                  <div>
                    <div className="flex items-center gap-3 mb-1">
                      ...
                      <span className={`px-2.5 py-0.5 rounded-full text-xs font-medium border ${
                        ...
                        shipment.status === "In Transit" ? "bg-blue-500/10 text-blue-600 dark:text-blue-400 border-blue-500/20" :
                        shipment.status === "Delayed" ? "bg-red-500/10 text-red-600 dark:text-red-400 border-red-500/20" :
                ...
                <div className="flex-1 flex items-center gap-4 lg:px-8">
                  ...
                  <div className="hidden md:flex flex-col items-center flex-1 px-4">
                    <div className="w-full h-0.5 bg-border relative">
                      ...
                      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 p-1 bg-background rounded-full border border-border">
