...
export default function TrackingLanding() {
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col items-center justify-center p-4 relative overflow-hidden">
      {/* Background Elements */}
      <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-background via-background to-background z-0"></div>
      ...
      >
        ...
        <h1 className="text-5xl md:text-7xl font-bold tracking-tighter text-transparent bg-clip-text bg-gradient-to-b from-foreground to-muted-foreground">
          Track your World
        </h1>
        ...
        <p className="text-muted-foreground text-lg md:text-xl max-w-lg mx-auto leading-relaxed">
          ...
          <span className="block text-muted-foreground/80 text-sm mt-2">Enter your shipment ID to begin.</span>
        </p>

        <form onSubmit={handleSearch} className="relative max-w-md mx-auto mt-12 group">
          <div className="absolute inset-0 bg-blue-500/20 blur-xl rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
          <div className="relative flex items-center bg-card/80 border border-border rounded-full p-2 shadow-2xl backdrop-blur-xl transition-all duration-300 focus-within:border-blue-500/50 focus-within:ring-1 focus-within:ring-blue-500/50">
            ...
            <input
              ...
              className="flex-1 bg-transparent border-none text-foreground placeholder-muted-foreground focus:outline-none px-4 py-2 text-lg font-mono tracking-wider w-full"
            />
            <button 
              type="submit"
        ...
        <div className="pt-12 flex justify-center gap-8 opacity-50">
          {['Global Logistics Corp', 'Ultra-Track', 'Swift-Move'].map((brand, i) => (
            <span key={i} className="text-sm font-semibold text-muted-foreground uppercase tracking-widest">{brand}</span>
          ))}
        </div>
      </motion.div>
    </div>
