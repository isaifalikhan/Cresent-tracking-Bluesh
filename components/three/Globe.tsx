...
const MovingVehicle = ({ start, end, mid, type, speed = 0.002, isLight = false }: { start: THREE.Vector3, end: THREE.Vector3, mid: THREE.Vector3, type: 'car' | 'truck' | 'bike', speed?: number, isLight?: boolean }) => {
  ...
  return (
    <group ref={meshRef}>
      <Html center distanceFactor={15} style={{ pointerEvents: 'none' }}>
        <div 
          className={`flex items-center justify-center rounded-full border p-1 shadow-[0_0_10px_rgba(34,197,94,0.3)] backdrop-blur-sm ${
            isLight 
    ...
    <group ref={globeRef}>
      ...
      <group position={latLonToVector3(PAKISTAN_LAT, PAKISTAN_LON, GLOBE_RADIUS)}>
        ...
        <Html distanceFactor={12}>
          <div className={`flex items-center gap-2 text-[10px] px-2 py-1 rounded-md border backdrop-blur-md shadow-lg shadow-green-500/20 whitespace-nowrap select-none pointer-events-none translate-y-[-20px] ${
            isLight 
              ? "bg-white/90 text-slate-900 border-green-500/30" 
